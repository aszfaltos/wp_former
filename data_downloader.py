...
def download_data(path: str, from_time: str, to_time: str, period: int):
    download_omsz_data(os.path.join(path, 'omsz'), from_time, to_time, period)
    download_mavir_data(os.path.join(path, 'mavir'), from_time, to_time, period)
...
def main(args: list):
    ...
    download_data(args.path, args.from_time, args.to_time, args.period)
